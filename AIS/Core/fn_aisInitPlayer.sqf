...
if (isClass (configFile >> "CfgPatches" >> "ace_medical") || isClass (configFile >> "CfgPatches" >> "ace_main")) then {
	...
	_player setVariable ["AIS_noReviveInit", true];
};

waitUntil {!isNil {_player getVariable "AIS_noReviveInit"}};
if (_player getVariable ["AIS_noReviveInit", true]) exitWith {["AIS is disabled"] call BIS_fnc_logFormat};
["AIS InitPlayer"] call BIS_fnc_logFormat;
