	...
	while {true} do {
		WestUnits = [];
		{
			if ((side _x) == WEST) then {
				_leader = leader _x ;		//returns all west group leaders
				WestUnits pushBack _leader;
			};
		} forEach allGroups;   

		zbe_players = (switchableUnits + playableUnits + WestUnits);
		
		EastUnits = [];
//		{if ((side _x) == EAST) then {EastUnits pushBack _x}} forEach allUnits;   //returns all east groups

		{
			if ((side _x) == EAST) then {
				_y = _x;
				{
					if (_x != leader _y && isNull objectParent _x) then {    // NO LEADERS, NO GUNNERS
						EastUnits pushBack _x;
					};	
				} forEach (units _y);
			};	
		} forEach allGroups;
